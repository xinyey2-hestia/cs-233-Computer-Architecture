...
puzzle_data: .space 216
# starcoin_data:	.space	128
RADAR_data:	.space	128
...
#wait_loop:
#	add	$t3, $t3, 1
#	bge	$t3, 1500, end_start
#        j	wait_loop                                # jump to end_start
############################################################################3

# interrupt handler
.kdata
chunkIH:	.space	40	# space for registers
...
puzzle:
	...
	add  $s0, $a0, 0  #encrypted[0]
	add  $s4, $a0, 64 #key
	lb   $s5, 208($a0)#rounds
	lw   $s6, 212($a0)#unique_chars
	sub  $sp, $sp, 64
	move $s7, $sp		  #plaintext
	sub  $sp, $sp, 64
	move $s1, $sp			#out_str
