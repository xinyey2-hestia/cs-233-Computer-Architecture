...
cubic_mandelbrot_vector(float x[SIZE], float y[SIZE]) {
    ...
    float temp[4],tempxy1[4],temp2[4];
    __m128 acc, X, Y, MAG,ArrayX,ArrayY,X2,Y2,constant;
  ...
  constant = _mm_loadu_ps(temp2);
    ...
    for (int i = 0; i < SIZE; i +=4) {
        ...
        X= _mm_loadu_ps(tempxy1);
        Y=_mm_loadu_ps(tempxy1);
        X2= _mm_loadu_ps(tempxy1);
        Y2=_mm_loadu_ps(tempxy1);
        ...
        for (int j = 0; j < M_ITER; j ++) {
          ...
          //  float y1_squared = y1 * y1;
            ...
            X2 = _mm_sub_ps(_mm_mul_ps(X,X),_mm_mul_ps(constant,_mm_mul_ps(Y,Y)));
            X2 = _mm_mul_ps(X,X2);
            X2 = _mm_add_ps(X2,ArrayX);

            // Calculate the imaginary portion of (x1 + (y1*i))^3 + (x + (y*i))
            //y2 = y1 * (3 * x1_squared - y1_squared) + y[i];

            Y2 = _mm_sub_ps(_mm_mul_ps(constant,_mm_mul_ps(X,X)),_mm_mul_ps(Y,Y));
            Y2 = _mm_mul_ps(Y,Y2);
            Y2 = _mm_add_ps(Y2,ArrayY);
            // Use the resulting complex number as the input for the next
            // iteration
            //x1 = x2;
            //y1 = y2;
